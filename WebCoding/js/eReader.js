...
$("#mainViewer").click(function (event){
    ...
    // event.offsetX is the horizontal position of the mouse inside the element we have clicked on,
    // it will be between 0 and the width of the element
    ...
    if(event.offsetX
        < $(this).width()*0.3){
        // if we've clicked on the lef go back
        counter = counter - 1;
    } else {
        // if we've clicked on the right go forwards
        counter = counter + 1;
    }
    ...
    // we get the id of the chapter thumbnail we want by putting counter on the end of it.
